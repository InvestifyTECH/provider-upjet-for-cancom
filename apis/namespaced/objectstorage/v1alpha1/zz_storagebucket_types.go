// SPDX-FileCopyrightText: 2024 The Crossplane Authors <https://crossplane.io>
//
// SPDX-License-Identifier: Apache-2.0

// Code generated by upjet. DO NOT EDIT.

package v1alpha1

import (
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"

	v1 "github.com/crossplane/crossplane-runtime/v2/apis/common/v1"
	v2 "github.com/crossplane/crossplane-runtime/v2/apis/common/v2"
)

type StorageBucketInitParameters struct {

	// (String) Name of the availability class. One of singleDc or multiDc.
	// Name of the availability class. One of singleDc or multiDc.
	AvailabilityClass *string `json:"availabilityClass,omitempty" tf:"availability_class,omitempty"`

	// (String) Name of the bucket. The name must be unique GLOBALLY.
	// Name of the bucket. The name must be unique GLOBALLY.
	BucketName *string `json:"bucketName,omitempty" tf:"bucket_name,omitempty"`
}

type StorageBucketObservation struct {

	// (String) Name of the availability class. One of singleDc or multiDc.
	// Name of the availability class. One of singleDc or multiDc.
	AvailabilityClass *string `json:"availabilityClass,omitempty" tf:"availability_class,omitempty"`

	// (String) Name of the bucket. The name must be unique GLOBALLY.
	// Name of the bucket. The name must be unique GLOBALLY.
	BucketName *string `json:"bucketName,omitempty" tf:"bucket_name,omitempty"`

	// (String) The ID of this resource.
	ID *string `json:"id,omitempty" tf:"id,omitempty"`
}

type StorageBucketParameters struct {

	// (String) Name of the availability class. One of singleDc or multiDc.
	// Name of the availability class. One of singleDc or multiDc.
	// +kubebuilder:validation:Optional
	AvailabilityClass *string `json:"availabilityClass,omitempty" tf:"availability_class,omitempty"`

	// (String) Name of the bucket. The name must be unique GLOBALLY.
	// Name of the bucket. The name must be unique GLOBALLY.
	// +kubebuilder:validation:Optional
	BucketName *string `json:"bucketName,omitempty" tf:"bucket_name,omitempty"`
}

// StorageBucketSpec defines the desired state of StorageBucket
type StorageBucketSpec struct {
	v2.ManagedResourceSpec `json:",inline"`
	ForProvider            StorageBucketParameters `json:"forProvider"`
	// THIS IS A BETA FIELD. It will be honored
	// unless the Management Policies feature flag is disabled.
	// InitProvider holds the same fields as ForProvider, with the exception
	// of Identifier and other resource reference fields. The fields that are
	// in InitProvider are merged into ForProvider when the resource is created.
	// The same fields are also added to the terraform ignore_changes hook, to
	// avoid updating them after creation. This is useful for fields that are
	// required on creation, but we do not desire to update them after creation,
	// for example because of an external controller is managing them, like an
	// autoscaler.
	InitProvider StorageBucketInitParameters `json:"initProvider,omitempty"`
}

// StorageBucketStatus defines the observed state of StorageBucket.
type StorageBucketStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        StorageBucketObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:storageversion

// StorageBucket is the Schema for the StorageBuckets API. Create and manage S3 buckets. Bucket names must be globally unique.
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,managed,upjet-for-cancom}
type StorageBucket struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.availabilityClass) || (has(self.initProvider) && has(self.initProvider.availabilityClass))",message="spec.forProvider.availabilityClass is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.bucketName) || (has(self.initProvider) && has(self.initProvider.bucketName))",message="spec.forProvider.bucketName is a required parameter"
	Spec   StorageBucketSpec   `json:"spec"`
	Status StorageBucketStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// StorageBucketList contains a list of StorageBuckets
type StorageBucketList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []StorageBucket `json:"items"`
}

// Repository type metadata.
var (
	StorageBucket_Kind             = "StorageBucket"
	StorageBucket_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: StorageBucket_Kind}.String()
	StorageBucket_KindAPIVersion   = StorageBucket_Kind + "." + CRDGroupVersion.String()
	StorageBucket_GroupVersionKind = CRDGroupVersion.WithKind(StorageBucket_Kind)
)

func init() {
	SchemeBuilder.Register(&StorageBucket{}, &StorageBucketList{})
}
