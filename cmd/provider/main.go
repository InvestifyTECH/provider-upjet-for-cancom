...
import (
	...
	apisCluster "github.com/InvestifyTECH/provider-upjet-for-cancom/apis/cluster"
	apisNamespaced "github.com/InvestifyTECH/provider-upjet-for-cancom/apis/namespaced"
	"github.com/InvestifyTECH/provider-upjet-for-cancom/config"
	"github.com/InvestifyTECH/provider-upjet-for-cancom/internal/clients"
	controllerCluster "github.com/InvestifyTECH/provider-upjet-for-cancom/internal/controller/cluster"
	controllerNamespaced "github.com/InvestifyTECH/provider-upjet-for-cancom/internal/controller/namespaced"
	"github.com/InvestifyTECH/provider-upjet-for-cancom/internal/features"
	"github.com/InvestifyTECH/provider-upjet-for-cancom/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Upjet-for-CANCOM").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-for-cancom"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-for-cancom",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Upjet-for-CANCOM APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Upjet-for-CANCOM APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Upjet-for-CANCOM controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Upjet-for-CANCOM controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Upjet-for-CANCOM controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Upjet-for-CANCOM controllers")
	}
